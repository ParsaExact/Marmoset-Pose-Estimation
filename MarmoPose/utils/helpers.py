...
class BaseVideoThread(threading.Thread):
    ...
    def put_frame(self, frame) -> bool:
        """Queue a frame, giving up if the reader is asked to stop.

        A plain blocking ``put`` cannot be interrupted: once the queue fills up,
        the thread parks in it forever and never reaches the ``stop_event`` check
        at the top of the loop, so ``MultiVideoCapture.stop()`` hangs in ``join``.
        These threads are non-daemon, so that also stops the interpreter from
        exiting -- a consumer that dies mid-render leaves the process wedged with
        its traceback still buffered and unprinted.
...
class LocalVideoThread(BaseVideoThread):
    ...
    def run(self):
        ...
        for frame in self.container.decode(video=0):
            ...
            if self.do_cache:
                if not self.put_frame(frame.to_ndarray(format='bgr24')):
                    break
