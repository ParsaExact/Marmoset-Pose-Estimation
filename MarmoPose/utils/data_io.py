...
def load_points_bboxes_2d_h5(file_path: Path) -> np.ndarray:
    ...
    with h5py.File(file_path, 'r') as f:
        # Strip the dataset suffix rather than splitting on the first underscore --
        # camera names routinely contain underscores (e.g. 'D01_1min'), and splitting
        # would look for a 'D01_points' dataset that does not exist.
        keys = sorted({k.rsplit('_', 1)[0] for k in f.keys()
                       if k.endswith(('_points', '_bboxes'))})
        for name in keys:
