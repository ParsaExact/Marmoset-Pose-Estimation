...
from marmopose.calibration.boards import CalibrationObject, Checkerboard, CharucoBoard
...
class Calibrator:
    ...
    @staticmethod
    def get_calibration_board(config: Config) -> CalibrationObject:
        board_size = config.calibration['board_size']
        square_length = config.calibration['board_square_side_length']

        if config.calibration.get('board_type', 'checkerboard').lower() == 'charuco':
            # For a ChArUco board, board_size counts SQUARES (the interior corners
            # are one fewer in each direction), unlike the Checkerboard case below
            # where board_size counts interior corners directly.
            marker_length = config.calibration['board_marker_length']
            return CharucoBoard(squaresX=board_size[0],
                                squaresY=board_size[1],
                                square_length=square_length,
                                marker_length=marker_length,
                                marker_bits=config.calibration.get('marker_bits', 4),
                                dict_size=config.calibration.get('dict_size', 50))
