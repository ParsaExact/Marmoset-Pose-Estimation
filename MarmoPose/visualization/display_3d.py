...
def pad_to_even(image: np.ndarray) -> np.ndarray:
    """Pad an image to even width and height.

    libx264 with yuv420p cannot encode odd frame dimensions. The 3D canvas is
    however sized by whatever Open3D's offscreen window actually gives us, which
    is capped by the desktop work area -- on Windows, asking for 1080 rows can
    yield 1061, and that odd height propagates into the composite frame.

    ffmpeg then exits before the first frame is written, and skvideo surfaces the
    resulting closed pipe as a bare ``OSError: [Errno 22] Invalid argument`` with
    an empty stderr, which says nothing about the real cause.

    Every frame in a run has the same shape, so this keeps the frame size
    constant across the video.
    """
    height, width = image.shape[:2]
    if height % 2 == 0 and width % 2 == 0:
        return image
    return np.pad(image, ((0, height % 2), (0, width % 2), (0, 0)), mode='edge')
...
class Visualizer3D:
    ...
    def generate_video_3d(self, source_3d: str = 'original', start_frame_idx: int = 0, end_frame_idx: int = None, video_type: str = 'composite', fps: int = 25):
        ...
        # Without the finally, an error in the loop leaves the reader threads
        # blocked on a full queue; being non-daemon, they then keep the process
        # alive at interpreter shutdown and the traceback is never flushed.
        try:
            for frame_idx in trange(start_frame_idx, end_frame_idx, ncols=100, desc=desc_info, unit='frames'):
                ...
                if video_type == '3d':
                    ...
                    if images_2d is None:
                        raise RuntimeError(
                            f'Timed out waiting for 2D frames at frame {frame_idx}. '
                            f'Expected {end_frame_idx} frames in {self.videos_2d_dir}.')
                    images_2d = [cv2.cvtColor(img, cv2.COLOR_BGR2RGB) for img in images_2d]
                    img_out = self.combine_images(images_2d, img_3d) # ~10ms

                writer.writeFrame(pad_to_even(img_out))
        finally:
            writer.close()
